...
public class Swing : MonoBehaviour {
    ...
    public PhysicMaterial swingMaterial, characterMaterial;
    ...
    void Update() {
        if (Input.GetMouseButtonDown(0)) {
            player.gameObject.GetComponent<CapsuleCollider>().material = swingMaterial;
            StartGrapple();
        }
        else if (Input.GetMouseButtonUp(0)) {
            player.gameObject.GetComponent<CapsuleCollider>().material = characterMaterial;
            StopGrapple();
        }
    }
    ...
    void StartGrapple() {
        ...
        if (Physics.Raycast(m_camera.position, m_camera.forward, out hit, maxDistance, whatIsGrappleable)) {
            ...
            grappleTarget.transform.position = grapplePoint;
